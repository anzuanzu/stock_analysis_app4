...
def get_stock_info(stock_id):
    ...
    if stock_history.shape[0] >= 2:
        return stock_history.iloc[-1]["Close"], stock_history.iloc[-1]["Close"] / stock_history.iloc[-2]["Close"] - 1
    else:
...
if uploaded_file is not None:
    ...
    # 增加篩選功能
    st.header("篩選功能")
    eps_min_filter = st.number_input("近5年平均EPS(元)最小值：", value=-100.0, step=0.1)
    eps_max_filter = st.number_input("近5年平均EPS(元)最大值：", value=1000.0, step=0.1)
    div_min_filter = st.number_input("近5年平均合計股利(元)最小值：", value=0.0, step=0.1)
    ...
    std_max_filter = st.number_input("盈餘標準差最大值：", value=100.0, step=0.1)
    payout_min_filter = st.number_input("股利發放率最小值：", value=0.0, step=0.01)
...
if st.button("篩選"):
    if not filtered_data.empty:
        # 获取最新报价和涨跌幅
        filtered_data["最新報價"], filtered_data["漲跌幅"] = zip(
            *filtered_data["代號"].apply(get_stock_info))
        st.write(filtered_data[[
                 "代號", "名稱", "最新報價", "漲跌幅", "盈餘標準差", "近5年平均EPS(元)", "近5年平均合計股利(元)", "股利發放率"]])
    else:
